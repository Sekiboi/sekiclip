...
from clipwork.media_ops import find_ffmpeg, find_ffprobe
from clipwork.prefs import load_prefs, user_data_dir


def crash_log_path() -> Path:
    return user_data_dir() / "clipwork_crash.log"


def job_log_tail(max_lines: int = 40) -> str:
    path = user_data_dir() / "clipwork_jobs.log"
    if not path.is_file():
        return "(none)"
    try:
...
def build_report() -> str:
    ...
    fp = find_ffprobe()
    lines = [
        "### Clipwork diagnostics (anonymous)",
        "",
        ...
        "",
        "### Recent job log (`clipwork_jobs.log`)",
        "",
        "```",
        job_log_tail(),
        "```",
        "",
        "### Crash log (`clipwork_crash.log`)",
        "",
        "```",
        crash_log_tail(),
        "```",
