...
def find_ffmpeg() -> Path | None:
    env = os.environ.get("SEKICLIP_FFMPEG") or os.environ.get("FFMPEG")
    if env:
        p = Path(env)
        if p.is_file():
            return p
...
def find_ffprobe() -> Path | None:
    env = os.environ.get("SEKICLIP_FFPROBE") or os.environ.get("FFPROBE")
    if env:
        p = Path(env)
        if p.is_file():
            return p
...
def require_ffmpeg() -> Path:
    ...
    if not p:
        raise RuntimeError(
            ...
            "or place ffmpeg.exe under vendor/, or set SEKICLIP_FFMPEG."
        )
    return p
...
def require_ffprobe() -> Path:
    ...
    if not p:
        ...
        raise RuntimeError(
            "ffprobe not found. Install ffmpeg (includes ffprobe) or set SEKICLIP_FFPROBE."
        )
    return p
...
def inject_thread_flags(args: list[str]) -> list[str]:
    """Use all CPU cores for decode/encode when caller didn't set -threads.

    Same CRF/bitrate → same quality; threads only affect speed/latency.
    Inserted before the first ``-i`` so demux and filters can parallelize too.
    """
    if not args or "-threads" in args:
        return list(args)
    out = list(args)
    try:
        i = out.index("-i")
    except ValueError:
        return ["-threads", "0", *out]
    return out[:i] + ["-threads", "0"] + out[i:]
...
def run_ffmpeg(
    ...
    *,
    ...
    Multi-threaded by default (quality unchanged).
    """
    global _CURRENT_PROC
    clear_cancel()
    ffmpeg = str(require_ffmpeg())
    args = inject_thread_flags(args)
...
def format_bytes(n: int | float | None) -> str:
    """Human file size for UI (B / KB / MB / GB / TB)."""
    try:
        b = float(n or 0)
...
def free_disk_bytes(path: Path | str) -> int | None:
    """Free space on the volume that will hold ``path`` (None if unknown)."""
    try:
        import shutil as _shutil

        p = Path(path)
        # Prefer existing parent; walk up if dest file does not exist yet
        probe_dir = p if p.is_dir() else p.parent
        while not probe_dir.exists() and probe_dir != probe_dir.parent:
            probe_dir = probe_dir.parent
        usage = _shutil.disk_usage(str(probe_dir if probe_dir.exists() else p.anchor or "."))
...
def probe(path: Path | str) -> dict[str, Any]:
    ...
    Uses a larger probe window so big / oddly muxed files still report streams
    reliably, without reading the whole media.
    """
    path = Path(path)
    if not path.is_file():
    ...
    cmd = [
        ...
        "-v",
        "quiet",
        # Enough head for large MP4/MKV headers; still tiny vs multi‑GB media
        "-probesize",
        "50M",
        "-analyzeduration",
        "50M",
    ...
    try:
        ...
        raise RuntimeError("ffprobe timed out reading media header") from exc
...
def media_summary(path: Path | str) -> str:
    ...
    if size:
        try:
            parts.append(format_bytes(int(size)))
        except (TypeError, ValueError):
            pass
    if v:
...
def staging_path(final: Path | str) -> Path:
    ...
    return final.with_name(f".{final.stem}.sekiclip_tmp{final.suffix or '.mp4'}")
