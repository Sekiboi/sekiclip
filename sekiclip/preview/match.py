...
def build_audio_filter(
    *,
    ...
    When ``input_preseeked`` is True, the demuxer already seeked to ``start``
    (preview-only fast path with ``-ss`` before ``-i``). Atrim uses relative
    times from 0. Export must keep ``input_preseeked=False`` for exact cuts.
    """
    ...
    if input_preseeked:
        # Preview path: caller already used ``-ss`` / ``-t`` on the demuxer.
        # Only reset timestamps — do NOT atrim with absolute 0:N while PTS are
        # still source-absolute (that produced complete silence).
    ...
    # Output-time offset of this window from cut start (always source-based)
