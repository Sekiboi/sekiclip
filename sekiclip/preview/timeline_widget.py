...
Interaction:
  - Click / drag on track (including blue selection) → move playhead
  - Drag green / red handles → In / Out
  - Alt+drag on selection → move whole In–Out range
...
class RangeTimeline(tk.Canvas):
    ...
    - click/drag empty track or selection body to scrub playhead
    - Alt+drag selection to move the whole range
    - zoom via set_view(start, end) in seconds
    """

    PAD = 10
    H = 56
    # Hit targets (px) — playhead intentionally easier to grab than marks
    HIT_POS = 18
    HIT_MARK = 12
    ...
    def redraw(self) -> None:
        ...
        # Playhead (yellow) — draw last so it stays on top
    ...
    def _hit(self, x: float, *, alt: bool = False) -> str:
        """Resolve drag target. Default track click = playhead (NLE expectation)."""
        # Marks first (tight), then playhead (wide), then track
        for name, t, hit in (
            ("in", self.in_t, self.HIT_MARK),
            ("out", self.out_t, self.HIT_MARK),
            ("pos", self.pos, self.HIT_POS),
        ):
            if abs(self._t_to_x(t) - x) <= hit:
                return name
        # Alt+click inside selection → move whole range
        if alt and self._t_to_x(self.in_t) <= x <= self._t_to_x(self.out_t):
            return "range"
        # Everywhere else (including blue selection body) scrubs playhead
        return "pos"

    def _press(self, event: tk.Event) -> None:  # type: ignore[type-arg]
        alt = bool(event.state & 0x20000) or bool(event.state & 0x00020000)
        # Windows Alt is often 0x20000; also check keysym via focus — use state bit 17
        try:
            alt = alt or bool(int(event.state) & 0x20000)
        except Exception:
            pass
        # Tk on Windows: Mod1/Alt can be 0x20000; some builds use 0x0008 for Alt? Prefer event.state
        self._alt_range = bool(getattr(event, "state", 0) & 0x20000)
        self._drag = self._hit(event.x, alt=self._alt_range)
        t = self._x_to_t(event.x)
        if self._drag == "range":
            self._drag_offset = t - self.in_t
            return
        # Playhead: jump immediately so hold-drag feels instant
        if self._drag == "pos":
    ...
    def _double(self, event: tk.Event) -> None:  # type: ignore[type-arg]
        # Double-click: set playhead and force a full preview refresh (seek_end)
        t = self._x_to_t(event.x)
        self.pos = max(0.0, min(t, self.duration))
        self.redraw()
        if self.on_seek:
